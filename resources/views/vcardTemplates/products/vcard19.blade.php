            ...
            <div class="vcard-one__product py-3 mt-0">
                <div class="row justify-content-between" @if(getLanguage($vcard->default_language) == 'Arabic') dir="rtl" @endif>
                    ...
                    <div class="col-4 mt-6 text-center">
                        <a class="product-btn  text-decoration-none" href="{{ route('vcard.show', ['alias' => $vcard->url_alias]) }}"
                                ...
                                <a @if ($product->product_url) href="{{ $product->product_url }}" @endif
                                    ...
                                    <div class="card product-card px-2 border-0 w-75 w-sm-100 h-100 mx-auto">
                                        <div class="d-flex justify-content-center">
                                            <img src="{{ $product->product_icon }}" alt="profile" class="w-50 mt-3 text-center object-fit-contain" height="208px" loading="lazy"/>
                                        </div>
                                        <div class="mt-3">
                                            <h4>{{ $product->name }}</h4>
                                            ...
                                            <div class="product-text text-center fs-20 fw-5 py-1">
                                                @if ($product->currency_id && $product->price)
                                                    <span
                                                        class="product-price-{{ $product->id }}">{{ $product->currency->currency_icon }}{{ number_format($product->price, 2) }}</span>
                                                @elseif($product->price)
                                                    <span class="product-price-{{ $product->id }}">{{ getUserCurrencyIcon($vcard->user->id) }} {{ $product->price }}</span>
                                                @endif
                                            </div>
                                        </a>
                                            @if (!empty($product->price))
                                                <div class="my-5 text-center">
                                                    <button class="product-btn  buy-product" data-id="{{ $product->id }}">{{ __('messages.subscription.buy_now') }}</button>
                                                </div>
                                            @endif
                                        </div>
                                    </div>
