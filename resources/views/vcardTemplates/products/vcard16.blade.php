            ...
            <div class="vcard-one__product py-3 mb-10 mt-0">
                <div class="row m-0  mb-3" @if(getLanguage($vcard->default_language) == 'Arabic') dir="rtl" @endif>
                    ...
                    <div class="col-sm-3 text-end mt-2">
                        <a class="vcard-sixteen-btn text-decoration-none" href="{{ route('vcard.show', ['alias' => $vcard->url_alias]) }}"
                            ...
                            <div class="d-flex justify-content-center mb-2">
                                <a @if ($product->product_url) href="{{ $product->product_url }}" @endif
                                    ...
                                    <div class="card product-card border-0 w-75 h-100">
                                        ...
                                        <div class="product-desc card-body d-flex align-items-center justify-content-between" @if(getLanguage($vcard->default_language) == 'Arabic') dir="rtl" @endif>
                                            ...
                                            <div class="product-amount text-primary  fs-18">
                                                @if ($product->currency_id && $product->price)
                                                    <span
                                                        class="text-dark product-price-{{ $product->id }}">{{ $product->currency->currency_icon }}{{ number_format($product->price, 2) }}</span>
                                                @elseif($product->price)
                                                    <span class="text-dark product-price-{{ $product->id }}">{{ getUserCurrencyIcon($vcard->user->id) }}{{ $product->price }}</span>
                                                @endif
                                            </div>
                                        </div>
                                    </a>
                                    @if (!empty($product->price))
                                        <div class="mb-3 text-center ">
                                            <button class="send-btn btn-primary buy-product" data-id="{{ $product->id }}">{{ __('messages.subscription.buy_now') }}</button>
                                        </div>
                                    @endif
                                </div>
                            </div>
