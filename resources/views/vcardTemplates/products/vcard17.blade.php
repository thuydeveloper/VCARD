        ...
        <div class="main-content mx-auto w-100 overflow-hidden border">
            ...
            <div class="vcard-one__product py-3 mb-10 mt-0">
                <div class="row justify-content-between" @if(getLanguage($vcard->default_language) == 'Arabic') dir="rtl" @endif>
                    <div class="col-4">
                        <h4 class="vcard-seventeen-heading p-1 ms-3" style="font-size:26px">{{ __('messages.vcard.products') }}</h4>
                    </div>
                    <div class="col-4 text-center mt-2">
                        <a class="vcard-seventeen-btn text-white text-decoration-none" href="{{ route('vcard.show', ['alias' => $vcard->url_alias]) }}"
                            role="button">{{ __('messages.common.back') }}</a>
                    </div>
                </div>
                <div class="container">
                    <div class="g-4 product-slider overflow-hidden mt-5">
                            ...
                            <div class="d-flex justify-content-center mb-2">
                                <a @if ($product->product_url) href="{{ $product->product_url }}" @endif
                                    ...
                                    <div class="card product-card border w-75 h-100">
                                        <div class="product-profile d-flex justify-content-center mt-1">
                                            <img src="{{ $product->product_icon }}" alt="profile" class="w-50 mt-2 object-fit-contain"
                                                height="208px" loading="lazy"/>
                                        </div>
                                        <div class="bg-primary-light card-body p-5">
                                            <div class="d-flex justify-content-between" @if(getLanguage($vcard->default_language) == 'Arabic') dir="rtl" @endif>
                                                ...
                                                @if ($product->currency_id && $product->price)
                                                    <span
                                                        class="fs-18 fw-6 text-primary  product-price-{{ $product->id }}">{{ $product->currency->currency_icon }}{{ number_format($product->price, 2) }}</span>
                                ...
                                @if (!empty($product->price))
                                    <div class="mt-2 text-center ">
                                        <button class="btn send-btn rounded-2 btn-primary buy-product"
