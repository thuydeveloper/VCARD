                ...
                <div class="row justify-content-between"  @if(getLanguage($vcard->default_language) == 'Arabic') dir="rtl" @endif>
                    ...
                    <div class="col-4 mt-6 text-center">
                        <a class="btn product-btn text-decoration-none"
                            href="{{ route('vcard.show', ['alias' => $vcard->url_alias]) }}">{{ __('messages.common.back') }}</a>
                    </div>
                </div>
                                    ...
                                    <div class="card product-card p-2 border-0 w-75 h-100">
                                        ...
                                        <div class="product-details mt-3">
                                            ...
                                            @if ($product->currency_id && $product->price)
                                                <span
                                                    class="text-light  product-price-{{ $product->id }}">{{ $product->currency->currency_icon }}{{ number_format($product->price, 2) }}</span>
                                            @elseif($product->price)
                                                <span class="text-light  product-price-{{ $product->id }}">{{ getUserCurrencyIcon($vcard->user->id) }}{{ $product->price }}</span>
                                            @endif
                                            </a>
                                            @if (!empty($product->price))
                                            <div class="my-5 text-center ">
                                                <button class="btn btn-primary text-decoration-none rounded-2 buy-product" data-id="{{ $product->id }}">{{ __('messages.subscription.buy_now') }}</button>
                                            </div>
                                            @endif
                                        </div>
                                    </div>
